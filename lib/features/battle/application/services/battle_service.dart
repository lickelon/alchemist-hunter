...
class BattleService {
  ...
  BattleResult runAutoBattle({
    ...
    final int partyPower = config.party.fold<int>(
      0,
      (int sum, HeroProfile h) => sum + h.power,
    );
    final int potionBoost = config.potionLoadout.values.fold<int>(
      0,
      (int s, int v) => s + v,
    );
    final int stagePower =
        100 + (config.stageId.codeUnitAt(config.stageId.length - 1) * 3);
    final int score = partyPower + (potionBoost * 5);

    final bool success = score >= stagePower || _random.nextDouble() > 0.35;
    final int turns = success
        ? 8 + _random.nextInt(6)
        : 12 + _random.nextInt(8);
