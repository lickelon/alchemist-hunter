...
List<String> battleEnemyStatLines(BattleCombatStats stats) {
  return <String>[
    ...
    '속도 ${stats.speed}',
    '치확 ${battleChanceLabel(stats.critChance)}',
    '치피 ${battleChanceLabel(stats.critDamage)}',
    '명중 ${battleChanceLabel(stats.accuracy)}',
    '회피 ${battleChanceLabel(stats.evasion)}',
    '상태적중 ${battleChanceLabel(stats.statusAccuracy)}',
    '상태저항 ${battleChanceLabel(stats.statusResistance)}',
    '물관 ${battleChanceLabel(stats.physicalPenetration)}',
    '마관 ${battleChanceLabel(stats.magicalPenetration)}',
    '흡혈 ${battleChanceLabel(stats.lifesteal)}',
    '회복력 ${battleChanceLabel(stats.healingPower)}',
    '재생 ${battleChanceLabel(stats.regen)}',
    '마나 ${stats.maxMp}',
    '마나재생 ${stats.mpRegen}',
  ];
}
  ...
  final List<String> lines = <String>[
    ...
    ...enemy.skills.expand(
      (BattleSkillDefinition skill) =>
          _battleSkillLabels(skill, manaCost: enemy.stats.maxMp),
    ),
  ];
  if (lines.isEmpty) {
...
List<String> _battleSkillLabels(
  BattleSkillDefinition skill, {
