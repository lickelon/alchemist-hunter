...
| 항목 | 현재 상태 | 문제 |
| --- | --- | --- |
| 아군 전투 스탯 | `HeroProfile(id, name, power)` 단일 수치 중심 | 공격/방어/체력/속도/치명 같은 실제 전투 스탯이 없다 |
| 적 전투 스탯 | 별도 적 유닛 모델 없음 | 적 역할, 패턴, 개별 난이도, 개별 드롭을 설계할 수 없다 |
| 전투 루프 | `score >= stagePower` + 확률 보정 | 행동 순서, 타겟 선택, 피해 계산, 상태 효과, 스킬 개념이 없다 |
| 전투 결과 표시 | `lastSummary` 문자열 1개 | 근거와 세부 내역이 없다 |
| 결과 이력 | 없음 | 최근 1회 이상 추적 불가 |
| 드롭 근거 | `loot`만 합산 반영 | 어떤 적이 무엇을 드롭했는지 알 수 없다 |
| 스테이지 데이터 | drop table만 존재 | 권장 전투력, 적 세트, 해금 조건, 보상 곡선이 흩어져 있다 |
| 적 정보 | 없음 | stage별 적 구성과 난이도 설명이 불가능하다 |
| 적별 드롭 | 없음 | 드롭 테이블을 설명 가능한 구조로 보여줄 수 없다 |
| 포션 loadout | 하드코딩 값 사용 | UI도 없고 실제 소비도 없다 |
| sync 경계 | battle sync는 `BattleState`만 갱신 | 포션 소비 같은 cross-feature 반영이 어렵다 |
...
### 3.2 B단계: 적 유닛 / enemy set / 스테이지 데이터 정규화
...
- 포션 부족 시 전투는 계속 돌되, 로그에서 이유를 볼 수 있다.
...
## 4. 권장 구현 순서
1. A단계 아군 전투 스탯 모델 정규화
2. B단계 적 유닛 / enemy set / 스테이지 데이터 정규화
3. C단계 전투 루프 고도화
4. D단계 결과 이력 모델과 상세 UI
5. E단계 해금 조건 체계화
6. F단계 포션 loadout 상태와 UI
7. G단계 포션 소비 규칙과 sync 경계 정리
8. H단계 테스트 확장

## 5. 단계 분리 기준
- 1차 PR
  - A단계 + B단계 일부 스캐폴딩
  - 목적: 아군/적 전투 데이터 구조 확정
- 2차 PR
  - B단계 + C단계
  - 목적: 적 구성과 실제 전투 루프 도입
- 3차 PR
  - D단계 + E단계
  - 목적: 결과 가시성과 해금 규칙 정리
- 4차 PR
