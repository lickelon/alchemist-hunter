...
| 전투 결과 표시 | 최근 결과 시트와 실시간 전투 현황 시트 제공 | 해금 근거와 포션 소비 근거는 아직 없다 |
| 결과 이력 | stage별 최근 로그 10개 보존 | 로그에 해금 / loadout fallback 사유는 아직 없다 |
| 드롭 근거 | stage별 적 구성과 적별 드롭 표시 가능 | Stage 1~5 재료 분배 정책은 더 명확히 정리해야 한다 |
| 스테이지 데이터 | stage / enemy set / enemy catalog 분리 완료 | unlock 규칙은 여전히 일부 하드코딩이다 |
...
### 3.7 G단계: 포션 소비 규칙과 sync 경계 정리 [대기]
...
2. B단계 잔여 작업
   ...
   - 상점 전용 재료 제거
   - 재료의 stage 중복 제거
3. F단계 포션 loadout 상태와 UI
4. G단계 포션 소비 규칙과 sync 경계 정리
5. H단계 테스트 확장

## 5. 남은 PR 분리 기준
- 1차 PR
  - 해금 조건 catalog + stage 잠금 문구 / 판정 통합
  - 목적: Stage 2~5 해금 규칙 하드코딩 제거
- 2차 PR
  - Stage 1~5 적 세트 / 드롭 / 보상 / 재료 분배 구체화
  - 목적: stage progression과 재료 수급 경로 확정
- 3차 PR
  - 포션 loadout 상태와 편성 UI
  - 목적: stage별 포션 설정 저장
- 4차 PR
  - 포션 소비 / fallback / sync 경계
  - 목적: loadout 실사용 연결
- 5차 PR
  - 회귀 / 보강 테스트
