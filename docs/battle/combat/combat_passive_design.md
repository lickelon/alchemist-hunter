...
- 현재 실제 효과가 연결된 훅은 `battleStart`의 `firstStrike`, `beforeHitCheck`의 `alwaysHit`, `beforeAction` / `beforeDamage` / `afterHit` / `turnEnd`의 `grantModifier`, `grantStatus`, `grantShield`, `afterAction`의 `extraAttack` 파생 lifecycle 실행, `onDamaged`의 `counterAttack` 파생 lifecycle 실행이다.
- 복합 조건식과 고급 상태이상은 후속 확장 지점이다.
