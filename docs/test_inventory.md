...
- 세션 sync가 시간 가속을 반영해서 전투 탐색, 전투 재생, 대장간 진행을 실제로 앞당기는지 검증한다.
- 포션이 부족한 경우 현재 전투가 `loadout fallback` 상태로 시작되는지 검증한다.
...
- stage 상태 시트, 적 구성/드롭 시트, 최근 결과 시트까지 실제 UI 진입 흐름을 확인한다.
- 최근 결과 시트에서 재료명과 행동 로그가 올바르게 보이는지 확인한다.

#### `test/features/battle/presentation/viewmodels/battle_controller_test.dart`
- 즉시 전투 후 골드, 에센스, 재료, XP, 최근 로그가 함께 갱신되는지 검증한다.
