...
- stage별 encounter 풀이 최소 10개 이상이고, 실제 등장 가능한 적 종류가 최소 5종 이상이며, encounter 확률 합계가 1.0으로 유지되는지 확인한다.
...
- 광역 피해 스킬이 모든 적에게 같은 lifecycle에서 적용되는지 확인한다.
- 아군 전체 회복 스킬이 모든 아군 대상의 HP를 회복하는지 확인한다.
- modifier 부여 스킬이 적 대상에게 active modifier를 남기는지 확인한다.
- 상태이상 부여 스킬이 적 대상에게 status와 power를 남기는지 확인한다.
- 보호막 스킬이 모든 아군 대상에게 shield를 부여하는지 확인한다.
...
- 연속 run 기준 전투 현황 시트, 적 정보/드롭 시트, 최근 기록 시트까지 실제 UI 진입 흐름을 확인한다.
