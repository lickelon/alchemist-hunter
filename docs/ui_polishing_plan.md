...
- 텍스트를 줄이고, 상태는 badge / chip / icon 같은 시각 요소로 바꾼다.
- 리스트는 상세 설명이 아니라 현재 대상, 상태, 가능한 액션만 보여준다.
- 숫자는 짧게 표시하고, 긴 근거 설명은 상세 모달로 보낸다.
- 모달, 바텀시트, 그리드, 빈 상태는 공통 규칙을 우선 적용한다.
- 테스트로 디자인 금지 규칙을 강제하지 않는다.

## 2.1 폴리싱 방향
현재 UI는 게임 화면보다 관리 도구처럼 텍스트 설명이 많다.
폴리싱의 핵심은 텍스트를 보기 좋게 줄바꿈하는 것이 아니라, 화면에서 읽어야 하는 문장을 줄이고 정보 위계를 다시 세우는 것이다.

기준:
- 제목은 대상 이름 또는 결과 이름을 우선한다.
- 상태는 `진행`, `대기`, `완료`, `잠금`, `부족`처럼 짧은 badge / chip으로 표시한다.
- 리스트 subtitle에는 긴 설명을 넣지 않는다.
- `A / B / C` 연결 문구는 가능하면 행, chip, 아이콘, 모달 상세로 분리한다.
- 같은 정보가 제목, subtitle, 버튼에 반복되면 하나만 남긴다.
- 완료 항목은 `완료` chip과 `수령` 버튼만으로 충분하면 결과 설명을 숨긴다.
- 상세 비용, 상세 효과, 근거 설명은 리스트가 아니라 상세 모달에서 보여준다.
...
- 그리드는 최대 칸 수 기준의 전체 박스만 중앙에 두고, 내부 아이템은 항상 왼쪽부터 채워야 한다.
- 그리드 칸은 최소 크기를 유지하되 남는 폭이 있으면 칸 크기를 키워 행 폭을 채워야 한다.
...
구현 계획:
- `ResourceIconGrid`는 옵션 없이 내부 아이템을 항상 왼쪽부터 채우도록 유지한다.
- `ResourceIconGrid`는 사용 가능한 폭에서 최대 칸 수를 계산하고, 남는 폭은 칸 크기에 분배한다.
...
1차 적용 범위:
- `ResourceIconGrid` 중앙 박스 + 내부 왼쪽 채움 + 칸 크기 확장
...
방향:
- Workshop은 텍스트 설명을 보기 좋게 재배치하는 화면이 아니라, 아이콘 / 상태 / 액션 중심의 게임 UI로 줄이는 대상이다.
- 리스트는 상세 설명 영역이 아니라 현재 대상, 상태, 가능한 액션만 보여준다.
- 자원, 재료, 원소, 포션, 장비처럼 고르는 대상은 아이콘 그리드를 우선한다.
- 작업 선택, 스킬 선택, 보조 슬롯처럼 판단과 액션이 함께 필요한 화면은 리스트를 유지하되 subtitle을 최소화한다.
- 비용, 효과, 품질 근거, 제작 시간 같은 상세 정보는 선택 후 모달이나 상세 영역으로 보낸다.
- tooltip은 이름과 핵심 수량 정도만 담고, 긴 설명을 넣지 않는다.
...
화면별 기준:
- 연금술 시트: 양조 / 제작 / 레시피북 / 연구의 역할을 탭 안에서 명확히 나누고, 각 탭의 첫 화면은 선택 가능한 대상 중심으로 둔다.
- 양조 실험: 선택한 두 원소, 메인 비율, 실험 가능 여부만 즉시 판단하게 한다.
- 레시피북: 발견한 포션은 그리드로 보여주고, 품질 / 최대 제작 / 비율 상세는 모달로 보낸다.
- 제작 상세: 결과 아이콘과 제작 수량을 상단에 두고, 필요 재료는 그리드, 비용과 시간은 보조 정보로 둔다.
- 추출 상세: 재료 아이콘, 보유량, 추출 수량, 추출 프로필 순서로 둔다.
- 작업 큐: 작업명, 짧은 상태 chip, 남은 시간 또는 수령 버튼만 기본 노출한다.
- 인챈트 / 부화 / 보조 / 스킬트리: 리스트는 유지하되 긴 설명 대신 chip과 짧은 액션으로 압축한다.
...
확인된 이슈:
- ~~작업 큐 리스트 subtitle에 `제작 / 수령 대기\n결과` 형태의 줄 연결이 남아 있다.~~ 완료
- ~~작업 큐 리스트는 정보를 줄 단위로 더 늘리는 방식이 아니라, 제목 + 상태 chip + 최소 시간 정보로 줄여야 한다.~~ 완료
- ~~연금술 시트 header에 큐 경고와 탭이 같이 있어 위계가 약하다.~~ 완료
- ~~제작 상세 모달의 `결과`, `제작 수량`, `필요 재료`, `소요 시간`이 같은 텍스트 무게로 표시된다.~~ 완료
- ~~추출 상세와 제작 상세의 섹션 제목 / 간격 기준이 완전히 같지 않다.~~ 완료

구현 계획:
- ~~작업 큐 리스트에서 `job.typeLabel`, 긴 `job.statusText`, 긴 `job.resultText`를 기본 노출하지 않는다.~~ 완료
- ~~작업 큐 리스트는 `compactTitle`, 짧은 상태 chip, 진행 / 대기 시간 정도만 표시한다.~~ 완료
- ~~완료 작업은 `완료` chip과 `수령` 버튼을 우선 노출하고, 결과 설명은 제목이 충분하면 숨긴다.~~ 완료
- ~~연금술 시트의 큐 가득 참 경고를 탭과 시각적으로 분리한다.~~ 완료
- ~~제작 상세 모달 상단에 결과 아이콘과 결과 수량을 summary로 강화한다.~~ 완료
- 제작 시간, 정수, 신비, 원소 비용은 상세 모달 안에서만 간결하게 정리한다.
- ~~추출 상세도 같은 섹션 제목 스타일과 간격 기준을 적용한다.~~ 완료

다음 적용 후보:
1. Battle stage 카드 summary 정리
2. Battle 보상 수령 모달의 `재료 없음` 표현 정리
3. Battle 전투 현황 시트 spacing / 카드 순서 점검

완료 기준:
- 주요 작업은 한 화면에서 등록 가능 여부와 비용을 바로 판단할 수 있다.
- 상세 정보는 모달로 분리되며, 리스트 본문에 긴 설명이 쌓이지 않는다.
- 상태와 액션은 텍스트 설명보다 chip / 버튼 / 아이콘으로 먼저 인식된다.
...
- ~~`재료 없음`은 공통 empty/보조 텍스트 패턴과 맞춰볼 수 있다.~~ 완료
- ~~`DungeonScreen` stage 카드 summary가 `편성 N명\n상태: X` 단일 텍스트다.~~ 완료
- ~~전투 현황 시트는 기능보다 카드 간 정보 우선순위 점검이 필요하다.~~ 점검 완료

구현 계획:
- ~~stage 카드 summary를 `DetailLines` 또는 행 구조로 분리한다.~~ 완료
- 보상 수령 모달은 수령 예정 / 이미 반영된 경험치 구분을 유지한다.
- ~~재료 없음 표현만 공통 빈 상태 또는 약한 보조 텍스트로 정리한다.~~ 완료
- ~~전투 현황 시트는 spacing과 카드 순서만 점검하고 기능 변경은 하지 않는다.~~ 점검 완료
...
확인된 이슈:
- ~~상점 header에 상태 정보와 `강제 갱신` 액션이 같이 있다.~~ 완료
- ~~상점 item subtitle에 `가격 / 품절\n다음 재입고` 문자열 연결이 남아 있다.~~ 완료
- ~~포션 판매 상세 모달은 `Text + SizedBox` 반복이며 `DetailLines`를 쓰지 않는다.~~ 완료
- 장비 상세 모달은 badge 나열만 있어 장비명 / 슬롯 / 효과 위계가 약하다.
- 대장간 `장비 등록` 리스트는 제작 비용, 시간, 효과 badge가 한 줄에 같이 몰려 있다.

구현 계획:
- ~~상점 header는 구매 제한 / 재입고 상태만 표시한다.~~ 완료
- ~~강제 갱신 버튼은 `AppSheetLayout.footer`로 이동한다.~~ 완료
- ~~상품 subtitle은 `DetailLines`로 가격, 재고, 품절, 재입고를 분리한다.~~ 완료
- ~~포션 판매 상세는 `DetailLines`로 교체한다.~~ 완료
- 장비 상세 모달은 아이콘, 슬롯, 효과 정보를 구분해 표시한다.
- 대장간 `장비 등록` 리스트는 기본 노출을 이름, 슬롯, 등록 액션 중심으로 줄이고 상세 정보는 모달로 보낸다.
- ~~용병 고용은 이미 footer에 후보 갱신이 있으므로 후순위 점검으로 둔다.~~ 점검 완료
