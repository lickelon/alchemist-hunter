...
- 작업 큐 리스트는 정보를 줄 단위로 더 늘리는 방식이 아니라, 제목 + 상태 chip + 최소 시간 정보로 줄여야 한다.
...
구현 계획:
- 작업 큐 리스트에서 `job.typeLabel`, 긴 `job.statusText`, 긴 `job.resultText`를 기본 노출하지 않는다.
- 작업 큐 리스트는 `compactTitle`, 짧은 상태 chip, 진행 / 대기 시간 정도만 표시한다.
- 완료 작업은 `완료` chip과 `수령` 버튼을 우선 노출하고, 결과 설명은 제목이 충분하면 숨긴다.
