...
- ~~장비 상세 모달은 badge 나열만 있어 장비명 / 슬롯 / 효과 위계가 약하다.~~ 완료
- ~~대장간 `장비 등록` 리스트는 제작 비용, 시간, 효과 badge가 한 줄에 같이 몰려 있다.~~ 완료
...
- ~~장비 상세 모달은 아이콘, 슬롯, 효과 정보를 구분해 표시한다.~~ 완료
- ~~대장간 `장비 등록`은 그리드로 표시하고 상세 정보는 모달로 보낸다.~~ 완료
