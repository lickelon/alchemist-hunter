...
### ⚠️ 6-1. `workshopQueueCardSummaryProvider.description` 미사용
...
### ⚠️ 6-3. `WorkshopJobType.craft`에 `'제조'` 레이블 혼용
...
### ⚠️ 6-5. `_BrewRatioSlider` `메인`/`서브` 레이블 어색함
...
### ⚠️ 6-7. 양조 상세 다이얼로그에서 `'양조 가능 X회'` 위치
...
### ⚠️ 7-4. 부화 시트 header — 보유 호문쿨루스 수 불필요
...
### ⚠️ 7-7. `_DetailRows` — `DetailLines`의 중복 구현
...
### ⚠️ 7-9. `BattleClaimDialog._ClaimSection` — `SectionCard`/`DetailLines` 미사용 독자 구현
...
### ⚠️ 7-10. `CharacterCombatEffectSection` — `DetailLines` 미사용 독자 구현
...
### ⚠️ 7-11. 인챈트 미리보기 — 현재/예상/변화량이 동일 weight
...
### ⚠️ 7-12. 스킬트리 노드 다이얼로그 — 현재/다음 효과 레이블 구분 없음
...
### ⚠️ 7-13. 보조 슬롯 header — 배치 수와 요약 문장이 동일 weight
...
### ⚠️ 7-14. 작업실 스킬트리 header — `'신비 N'` 레이블 없이 단독 수치
...
| 우선순위 | 이슈 |
|---|---|
| 높음 | 6-4. 원소 1개 선택 시 안내 메시지 중복 |
| 높음 | 6-3. craft 타입 레이블 '제조'/'양조'/'제작' 혼용 |
| 높음 | 7-5. 인챈트 확인 다이얼로그 `\n` 텍스트 |
| 높음 | 7-6. 인챈트 등록 버튼이 body 안에 위치 |
| 높음 | 7-7. `_DetailRows` `DetailLines` 중복 구현 |
| 중간 | 6-1. description 필드 계산 후 미사용 |
| 중간 | 6-2. 카드 이름과 시트 제목 불일치 |
| 중간 | 6-7. 양조 가능 횟수 위치 |
| 중간 | 6-8. 소요 시간 표기 불명확 |
| 중간 | 7-3. 보상 수령 다이얼로그 런 요약 위계 |
| 중간 | 7-8. 보조 슬롯 현황 `/` 연결 텍스트 |
| 중간 | 7-9. `_ClaimSection` `SectionCard`/`DetailLines` 미사용 |
| 중간 | 7-10. `CharacterCombatEffectSection` `DetailLines` 미사용 |
| 중간 | 7-11. 인챈트 미리보기 현재/예상/변화량 동일 weight |
| 중간 | 7-12. 스킬트리 노드 현재/다음 효과 구분 없음 |
| 낮음 | 6-5. 메인/서브 레이블 |
| 낮음 | 6-6. 수량 슬라이더 레이블 배치 |
| 낮음 | 6-9. 빈 큐 상태 표현 불일치 |
| 낮음 | 6-10. 큐 가득 참 경고 스타일 없음 |
| 낮음 | 6-11. tooltip 수량 단위 없음 |
| 낮음 | 7-4. 부화 시트 header 호문쿨루스 수 |
| 낮음 | 7-13. 보조 슬롯 header 배치 수/요약 혼합 |
| 낮음 | 7-14. 스킬트리 header 수치 맥락 없음 |
