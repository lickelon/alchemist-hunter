...
- [x] UI 강제 갱신 버튼 연결
  - 파일: `lib/screens/weapons_screen.dart`
- [ ] 상점 기능을 마을 화면으로 이전(구매/판매 통합)
- [ ] 자동 갱신 스케줄러(실시간 주기 만료 시 자동 리롤)
- [ ] 상점별 구매 제한/품절 재입고 정책
...
- [ ] 기존 `lib/screens/dungeon_screen.dart`를 `전투` 탭 구조에 맞춰 라우팅/상태 정리
...
### M2 운영 코어 루프 (진행중)
- [-] 제조 품질 점수 계산 + 완제품 인벤토리 반영
- [-] 포션 타입 판정(조합/비율 분기) 실동작
- [-] 마을/작업실 분리 화면 기준 기능 이전
- [-] 기존 UI 마이그레이션(Weapons 분해, 구 화면 진입 경로 차단)
- [ ] 아키텍처/폴더 마이그레이션 1차(`features/game` 분해 이전)
- [ ] 상점 자동 갱신 스케줄러
...
## 8) 즉시 다음 작업(실행 순서)
1. `M2 마감: 포션 품질/타입 판정 + 완제품 인벤토리 반영`
2. `M2 마감: 마을/작업실 기능 이전 + 상점 자동 갱신`
3. `M3 착수: Level/Rank/Tier 성장 플로우 + 티어 승급 재료`
4. `M3 착수: 특수 재료 기반 해금 UI 완성`
