...
- [x] 작업실 중앙 큐 1개 + 통합 수령 구조 전환
  - `extraction / craft / enchant / hatch` 작업을 공용 큐로 통합
  - 입력 자원은 등록 시 선예약
  - 완료 결과는 `pendingClaim`에 적재 후 통합 수령
...
## 8) 즉시 다음 작업(실행 순서)
1. `M5 잔여: 영속 저장소 없이 동작하는 claim/sync UX 점검`
2. `M5 착수: 로컬 저장소 스키마/복귀 복원 설계`
3. `M3 잔여: 티어업 재료 획득 경로 연결`
4. `M4 잔여: 스킬트리 리스펙 정책 정리`
5. `M6 준비: 밸런스 테이블 외부화 범위 확정`
