...
## 0.1) 초기 기획(v1) 대비 현재 구현 차이
- [x] 전투 중심 루프 -> 운영 중심 루프로 전환
  - v1: `영웅 편성 -> 던전 자동 전투 -> 성장`
  - 현재 구현 기준: `재료 수급 -> 작업실 제조 -> 판매/전투 투입 -> 성장`
- [x] 탭 구조 전면 변경
  - v1: `Characters / Weapons / Dungeons / Pets`
  - 현재 구현 기준: `마을 / 작업실 / 캐릭터 / 전투`
- [x] 캐릭터 체계 재정의
  - v1: 영웅 + 펫 중심
  - 현재 구현 기준: `용병 + 호문쿨루스` 공통 성장축(`Level / Rank / Tier`)
- [x] 성장 핵심 수단 변경
  - v1: 장비 드롭/특성/펫 확장 중심
  - 현재 구현 기준: 전투 XP 성장 + 작업실 포션 제작/해금 + 티어 재료 소모 중심
- [-] 전투 설계 단순화
  - v1: 턴제 자동 전투 엔진, 오염도, 위협도 타게팅, 전투 로그 상세
  - 현재 구현 기준: 완전 자동 전투 결과 처리 우선, 상세 전투 규칙은 아직 축소/미구현
- [ ] 장비/펫/환경 변수 확장 요소 보류
  - v1의 장비 40종, 펫 6종, 오염도 시스템은 현재 MVP 구현 범위 밖
- [x] 작업실 큐 정책 변경
  - 초기 구상 단계의 실패/재시도 뉘앙스 대신
  - 현재 구현 기준: `등록 불가 / 진행 불가(blocked) / 재개` 정책으로 단순화

## 1) 제품 방향 (확정)
- 운영(제조/판매) 중심 방치형
- 코어 루프: 재료 수급 -> 특성 추출/조합 -> 포션 제조 -> 판매/전투 투입 -> 성장
- 전투는 스테이지별 지속 원정형(보조 루프)
- 스킬트리는 `마을 트리 / 작업실 트리`로 분리하고, 각 트리는 별도 재화로 강화
- 세션 목표: 초반 10~15분 -> 후반 3~5분 점검형
- 이세계 포탈은 시즌2 이관
- 시간 구조:
  - 전투: 스테이지별 개별 수령
  - 작업실: 중앙 큐 1개 + 작업별 개별 수령
  - 마을 장비 제작: 대장간 큐 + 장비별 개별 수령
...
- [x] 큐 차단/재개 정책 정리
  - 랜덤 실패 제거
  - `진행 불가(blocked)` 상태로만 차단 표현
  - 등록 전 재료 검증 + 진행 중 재료 부족 시 차단
- [x] 큐 등록 UX 개선
  - 고정 `x3` 제거, `1/3/5/최대` 수량 선택
  - 필요 trait/최대 제작 가능 횟수 표시
- [x] 작업실 UI 위젯 테스트 추가
  - 큐 차단/재개 문구 검증
  - 아이템 목록 이름/특성 요약 표시 검증
- [x] 작업실 중앙 큐 1개 + 작업별 개별 수령 구조 전환
  - `extraction / craft / enchant / hatch` 작업을 공용 큐로 통합
  - 입력 자원은 등록 시 선예약
  - 완료 작업은 수령 전까지 큐를 계속 점유
  - 완료 결과는 작업 행별로 개별 수령
...
- [x] 스테이지별 전투 편성
  - 스테이지마다 별도 편성 저장
  - 편성된 캐릭터만 전투력 계산/전투 XP 획득
- [x] 스테이지별 지속 원정 + 개별 수령
  - `BattleExpeditionState` 도입
  - `원정 시작 / 정지 / 수령` 흐름으로 전환
  - 보상은 pending 상태로 누적 후 스테이지별 개별 수령
- [-] 전투 화면 파티 전투력 표시
  - 현재: 스테이지별 `편성 수 + 전투력` 노출
...
- [x] 공통 시간 동기화 파이프라인 초안
  - `SessionState.lastSyncAt`
  - `SessionProgressSyncUseCase`
  - 상점 자동 갱신 + 전투 원정 + 작업실 중앙 큐 + 대장간 큐 공통 sync
- [-] 저장소 없이 동작하는 라이브 sync 연결
  - [x] `session_sync_scope.dart`가 세션 전체 sync 호출
  - [ ] 앱 종료/재실행 복구를 위한 영속 저장소 연동
- [ ] 영속 저장소(Hive/Isar) 도입
...
- [x] 마을 제작 서비스
  - 입력: 장비 블루프린트, 재료
  - 출력: 인챈트 전 장비 인스턴스
  - 현재는 즉시 지급이 아니라 `대장간 큐 등록 -> 완료 후 개별 수령`
- [x] 작업실 인챈트 서비스
  - 입력: 장비 인스턴스, 포션
...
- [-] 인챈트 규칙
  ...
  - [x] 강화 결과 미리보기 / 교체 확인 UX

### 3.7 용병 고용 시스템 (신규)
- [-] 용병 도메인 모델
  - [x] `MercenaryTemplate`: 기본 직업/고용 비용/티어
  - [x] `MercenaryCandidate`: 실제 고용 후보
  - [ ] `MercenaryInstance`: 계약/상태 포함 전용 보유 모델
- [-] 마을 고용 서비스
  - [x] 고용 후보 생성/갱신
  - [x] 골드 소모 후 고용
  - [ ] 해고 시 보상 규칙 정의
...
- [x] 호문쿨루스 티어 정의(4단계 고정)
  ...
  - 4: `Rubedo (적화)`
- [x] 편성 규칙
  - [x] 전투 파티에 용병/호문쿨루스 혼합 편성 가능
  - [x] 전투는 스테이지별 편성 사용
  - [x] 작업실 보조 슬롯은 호문쿨루스 전용 배치(추출/제조/인챈트/부화)
  - [x] 작업실 동시 배치 수는 3체 제한
  - [x] 기존 배치에서 제거해야만 다른 전투/작업실 슬롯으로 재배치 가능
- [x] 성장 규칙
...
- [-] 호문쿨루스 부화 규칙
  - [x] 부화는 작업실에서만 수행
  - [-] 부화 재료/시간/촉매 규칙 정의
    - [x] `Essence + ArcaneDust + 재료 + 추출 trait` 기반 즉시 부화 MVP
    - [ ] 시간 진행형 인큐베이션/촉매 확장 규칙
...
- [-] 완료 기준
  - [x] 마을 트리/작업실 트리 각각 최소 1개 노드 효과가 실제 수치에 반영
  - [x] 해금 불가 노드는 트리별 상태 문구로 원인 표시
  - [ ] 트리형 시각화/툴팁 정리
...
- [-] 스킬트리 진입/노드 강화 화면
  - [x] 마을 트리 노드 목록 표시
  - [x] 노드 상태(잠김/해금가능/강화가능/최대) 표시
  - [x] 강화 재화: `TownInsight` + 일부 노드 `Gold` 추가 요구
  - [ ] 트리 연결선 시각화
  - [x] 노드 선택 시 효과 미리보기(현재 -> 다음 레벨)
- [-] 마을 전용 재화 HUD
  - [x] `Gold`, `TownInsight`, 해금 노드 수 표시
  - [ ] `Gold` 잔액/증감 로그 표시
  - [x] `Essence`는 비노출
...
- [x] 스테이지 실행 버튼
- [x] 스테이지별 편성 시트
  - 전투 탭에서 스테이지별 캐릭터 배치/해제
  - 편성 0명일 때 전투 실행 차단
- [-] 잠금 상태 시 진입 제한/해금 조건 표시
  - 현재: 리스트 subtitle 문구로 잠금 사유 표시
  - 미완료: 툴팁/상세 모달 수준 안내
- [ ] 전투 결과 상세 모달
...
- [-] 호문쿨루스 목록: 성장 단계, 친화 특성, 보조 효과 표시
  - [x] 성장 단계(`Level / Rank / Tier`) 표시
  - [x] 출처 레시피, 역할, 보조 효과 표시
  - [ ] 친화 특성 표시
- [-] 캐릭터 상세에서 전투/작업실 배치 전환 가능
  - [x] 전투 배치 상태 확인
  - [ ] 작업실 배치 전환
- [-] 성장 패널
  - [x] 현재 `Level / Rank / Tier` 표시
  - [x] 랭크업 가능 조건 충족 시 수동 랭크업 버튼 표시
  - [x] 티어업 가능 조건 충족 시 수동 티어업 버튼 표시
  - [-] 티어업 재료 요구량/보유량 표시(조건 문구 수준만 반영)
  - [-] 다음 목표 표시(랭크/티어 조건 문구 수준만 반영)

### 4.6 작업실(Workshop) 화면 책임 (신규 확정)
- [-] 작업실 대시보드 카드 구조 정리
  - [x] 첫 화면 카드 순서를 `Queue -> Extraction -> Craft -> Enchant -> Hatch -> Inventory -> Support -> Skill Tree`로 재배치
  - [x] `Craft Queue`는 상태 확인/개별 수령 전용으로 정리
  - [x] `Craft` 카드를 별도 분리하여 포션 제조 등록 진입점으로 사용
  - [x] `Inventory` 카드로 재료/추출 특성/포션 스택 조회를 통합
  - [x] 첫 화면에서 `Material`/`Crafted Potions`/`Logs` 카드 제거
  - [ ] 추후 작업실 카드 상세 내용/요약 문구 UX 재정리 필요
- [-] 작업실 스킬트리 진입/노드 강화 화면
  - [x] 작업실 트리 노드 목록 표시
  - [x] 노드 상태(잠김/해금가능/강화가능/최대) 표시
  - [x] 강화 재화: `ArcaneDust` + 일부 노드 `추출 원소(Element)` 추가 요구
  - [ ] 트리 연결선 시각화
  - [x] 노드 선택 시 효과 미리보기(현재 -> 다음 레벨)
- [-] 작업실 전용 재화 HUD
  - [x] `Essence`, `ArcaneDust`, 해금 노드 수 표시
  - [ ] 시간당 변화량 표시
  - [x] `Gold`는 비노출
- [-] 보유 자원 조회 패널
  - [x] 보유 아이템(재료 중심)을 종류별 스택으로 묶어 수량 중심 표시
  - [x] 스택 셀 클릭 시 바텀시트 오픈
...
- [x] 제조 패널
  - 포션 선택 -> 목표 trait 비율 확인 -> `Craft` 카드에서 큐 등록
...
- [-] 인챈트 패널
  ...
  - [x] 강화 결과 미리보기 -> 적용
- [x] 작업실 보조 슬롯 패널
  - [x] 추출/제조/인챈트/부화 슬롯별 1체 배치
  - [x] 슬롯 효과/현재 배치 상태 표시
  - [x] 전투/다른 작업실 슬롯과 중복 배치 차단
- [-] 부화 패널(호문쿨루스)
  - [x] 부화식 선택 -> 자원 투입 -> 즉시 생성
  - [ ] 시간 진행 -> 결과 수령
- [ ] 공통 완료 기준
...
- [x] 제작 큐 상태 전이
- [x] 탭 렌더 위젯 테스트
- [x] 포션 타입 판정/품질 계산/인벤토리 소모
- [x] 전투 XP 성장/랭크업/티어업
- [x] 최대 레벨 XP 캡/표기
- [x] 작업실 위젯 테스트(큐 차단/재개, 보유 아이템 시트)
...
- [x] 용병 고용 use case/controller/widget 테스트
- [x] 마을/작업실 스킬트리 repository/use case/widget 테스트
- [x] 스킬트리 효과 적용 회귀 테스트
- [x] 호문쿨루스 부화 use case/widget 테스트
- [x] 스테이지별 전투 편성 use case/controller/widget 테스트
- [x] 배치 상태 캐릭터 프레젠테이션 테스트
- [x] 작업실 보조 슬롯 use case/widget 테스트
- [x] 전투/작업실 상호 배치 차단 회귀 테스트
...
- [x] 작업실 인챈트 골격
  ...
  - [x] 강화 결과 미리보기/교체 정책
- [x] 용병 고용 골격(마을 확보 -> 캐릭터 목록 연결)
- [x] 호문쿨루스 부화 MVP(작업실 큐 등록 -> 작업실 보상 수령)
- [x] 스테이지별 전투 편성 + 배치 상태 표시
- [x] 작업실 보조 슬롯 + 전투/작업실 상호 배타 배치 규칙
- [x] 방치형 시간 구조 1차 전환
  - 전투: 스테이지별 지속 원정 + 개별 수령
  - 작업실: 중앙 큐 1개 + 작업별 개별 수령
  - 마을: 대장간 큐 + 장비별 개별 수령
- [-] 특수 재료 기반 콘텐츠 해금 UI(잠김/조건 표시)
  - [x] 작업실 포션 등록 UI 잠금/조건 표시
  - [x] 전투 스테이지 잠금 조건 문구 확장
  - [x] 캐릭터 랭크업/티어업 조건 문구 확장
- [-] 성장 밸런스 1차 튜닝
  - [x] 전투 XP 스테이지 가중치 적용
  - [ ] 레벨/랭크 구간별 체감 곡선 추가 튜닝

### M4 이원화 스킬트리/재화 체계 (진행중)
- [x] 마을 스킬트리(Town) 구현: TownInsight + Gold 복합 비용
- [x] 작업실 스킬트리(Workshop) 구현: ArcaneDust + Element 복합 비용
- [-] 스킬트리 교차 강화 차단 + 트리별 리스펙 정책 반영
  - [x] 트리별 전용 재화/전용 controller 기반 교차 강화 차단
  - [ ] 트리별 리스펙 정책
...
### M5 저장/오프라인 통합 (진행중)
- [x] 세션 공통 sync 파이프라인 초안
- [x] 라이브 세션 sync 스코프 연결
- [ ] 로컬 저장소(Hive/Isar) 도입
...
## 8) 즉시 다음 작업(실행 순서)
1. `작업실 UI 후속: 카드 상세 내용/요약 문구 UX 재정리`
2. `M5 잔여: 영속 저장소 없이 동작하는 claim/sync UX 점검`
3. `M5 착수: 로컬 저장소 스키마/복귀 복원 설계`
4. `M3 잔여: 티어업 재료 획득 경로 연결`
5. `M4 잔여: 스킬트리 리스펙 정책 정리`
