...
## 8) 즉시 다음 작업(실행 순서)
1. `M3 마감: 작업실 보조 슬롯(호문쿨루스 우선) 구현`
2. `M3 마감: 용병 역할/계약 상태 표시 정리`
3. `M4 마감: 스킬트리 효과 미리보기/트리 시각화 정리`
4. `M5 준비: 로컬 저장소 스키마/마이그레이션 설계`
5. `M5 착수: 오프라인 진행 파이프라인 연결`
